...
module.exports = function(yasme, name) {
  return {
    ...
    get: function(token, callback) {
        ...
        if(rdfUtils.isWikidataPropertiesPrefix(yasme,prefix)){
          query.type='property';
        }else{
          delete query.type;
        }

        let endpoint = rdfUtils.getEndPoint(yasme,prefix);
        if(endpoint!=null){
...
module.exports.isValidCompletionPosition = function(yasme) {
  ...
  if(token.type == 'shape' || token.type=='string-2' || token.type=='constraint'){
    if(rdfUtils.isWikidataEntitiesPrefix(yasme,prefixName) 
      || rdfUtils.isWikidataPropertiesPrefix(yasme,prefixName)
      || rdfUtils.getEndPoint(yasme,prefixName)!=null){
        return true
    }
  }
